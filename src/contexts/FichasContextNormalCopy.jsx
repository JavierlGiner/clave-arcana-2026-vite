...
// import selectSound from "./modena_select.mp3";
// import swapSound from "./moneda_change.mp3";
...
const FichasProvider = ({ children }) => {
  ...
  const bloquearNuevaFicha = () => {
    const posiblesBloqueables = fichasRef.current.filter((ficha) => {
    ...
    console.log(posiblesBloqueables);
  ...
  useEffect(() => {
    const fichasActuales = fichasRef.current;
    ...
    if (fichaBloqueada) {
      ...
      if (opcionesHanCambiado) {
        ...
        const coincidencias = nuevasOpciones.filter(
          (opcion) => opcion.id === opcion.value
        );
